...
if ( ! class_exists( 'WTHP_Auto_Plugin_Update' ) ) {

class WTHP_Auto_Plugin_Update
{
	/**
	 ...
	 * @var string
	 */
	public $wsq_remote_path = 'http://updates.cmoreira.net/';
	...
	function __construct() {
		...
		$this->wsq_plugin_file = $folder_name.'/tshowcase.php';
...
new WTHP_Auto_Plugin_Update();


/* Custom - Hide Show Details for update, because it loads details from another plugin on WP.org */
add_action('admin_head', 'tshowcase_custom_admin_hack');

function tshowcase_custom_admin_hack() {
  echo '<style>
    #team-showcase-update .open-plugin-details-modal { display:none; }
    } 
  </style>';
}
